...
async function main() {
  ...
  const file = path.join(root, "database", "REVIEW_GALLERY_UPDATE.sql");
  if (!fs.existsSync(file)) throw new Error("ไม่พบ database/REVIEW_GALLERY_UPDATE.sql");
  ...
  const result = await sql`
    select category_id, count(*)::int as images
    from public.review_gallery_images
    group by category_id
    order by category_id
  `;

  console.log("ติดตั้งระบบภาพรีวิวแยก 3 หมวดเรียบร้อยแล้ว");
  console.table(result);
}

main().catch((error) => {
  console.error("ติดตั้งระบบภาพรีวิวไม่สำเร็จ");
