...
async function main() {
  ...
  const file = path.join(root, "database", "PORTFOLIO_BACKEND_UPDATE.sql");
  if (!fs.existsSync(file)) throw new Error("ไม่พบ database/PORTFOLIO_BACKEND_UPDATE.sql");
  ...
  console.log("เพิ่มตารางและดึงตัวอย่างผลงานเดิมเรียบร้อยแล้ว");
  console.table(result);
}

main().catch((error) => {
  console.error("อัปเดตตัวอย่างผลงานไม่สำเร็จ");
